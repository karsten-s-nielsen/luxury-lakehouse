...
Post-Task-2.4 (§C2 → C-b): fct_xg_predictions_v2 is no longer the ADR-013 v2 mart —
it is a BACK-COMPAT TABLE projecting fct_shot_xg (canonical-SPADL xg_model_v3)
into the exact legacy schema, so existing consumers (Taipy, the
fct_xg_predictions_v2_synced Lakebase table) keep working. It is materialized as
a TABLE (not a view) because a SNAPSHOT Lakebase synced table must source from a
Delta table. These tests assert that projection shape (materialization, bridge,
legacy columns, coverage restriction) and that the source declaration + contract
block are intact.
...
def test_mart_is_backcompat_table_over_shot_xg() -> None:
    ...
    # Now a TABLE projecting fct_shot_xg (not the ADR-013 v2 mart); a SNAPSHOT
    # synced table must source from a Delta table, not a view. Enforced contract kept.
    assert "materialized='table'" in flat
    assert "liquid_clustered_by=['match_key']" in flat
