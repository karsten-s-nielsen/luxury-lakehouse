"""Bronze SCHEMA-PARITY gate for Idsse: sources.yml must document the live schema.
...
(``fixtures/idsse_bronze_schema_snapshot.json``); refresh it when ingestion adds columns.
...
WHY THIS EXISTS
---------------
Idsse was one of the providers with no bronze-coverage gate at all — which is exactly how
...
grows: measured 2026-08-10, idsse carried 35 undocumented columns.

The table set comes from ``scripts/_bronze_table_inventory``, so this gate and the
non-contract classification cannot disagree about what idsse owns. Migration backups and
...
_SNAPSHOT_PATH = Path(__file__).resolve().parent / "fixtures" / "idsse_bronze_schema_snapshot.json"
_SOURCES_YML_PATH = sources_yml_path("idsse")
...
_EXPECTED_TABLES = contract_tables("idsse")
...
class TestIdsseBronzeCoverage:
    """Every live idsse bronze column must be documented in _idsse__sources.yml."""
