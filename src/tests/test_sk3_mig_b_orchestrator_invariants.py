...
def test_no_trainer_pins_silly_kicks_explicitly() -> None:
    ...
    The wheel's ``[spadl]`` extra (silly-kicks>=4.9.1,<5) is the single source
...
# ── §2.10.5 — every trainer declares _REQUIRED_SK_MIN = (4, 12, 0) ──────────
...
# Matches the pyproject [spadl] pin `silly-kicks>=4.12.0,<5`.


def test_all_trainers_assert_silly_kicks_runtime_min() -> None:
    """Each trainer must declare module-level `_REQUIRED_SK_MIN = (4, 12, 0)`.
    ...
    assert not missing, (
        f"Trainers missing module-level `_REQUIRED_SK_MIN: tuple[int, int, int] = (4, 12, 0)`: {missing}"
    )
    assert not wrong_value, f"Trainers with `_REQUIRED_SK_MIN` not equal to (4, 12, 0): {wrong_value}"
