...
# ── §2.10.5 — every trainer declares _REQUIRED_SK_MIN = (4, 22, 0) ──────────
...
def test_all_trainers_assert_silly_kicks_runtime_min() -> None:
    """Each trainer must declare module-level `_REQUIRED_SK_MIN = (4, 22, 0)`.
    ...
    assert not missing, (
        f"Trainers missing module-level `_REQUIRED_SK_MIN: tuple[int, int, int] = (4, 22, 0)`: {missing}"
    )
    assert not wrong_value, f"Trainers with `_REQUIRED_SK_MIN` not equal to (4, 22, 0): {wrong_value}"
