...
def _fetch_target_vector(
    ...
    @st.cache_data(ttl=get_settings().cache_ttl_seconds, show_spinner="Loading player vector...")
    def _query(tbl: str, pid: str, comp_id: int | None) -> Any:
    ...
    return _query(table, str(player_id), int(competition_id) if competition_id is not None else None)
...
def _search_similar_players(
    ...
    @st.cache_data(ttl=get_settings().cache_ttl_seconds, show_spinner="Finding similar players...")
    def _query(
        tbl: str,
        vec_str: str,
        vec_col: str,
...
def _load_radar_stats(canonical_player_ids: list[str], competition_id: int | None) -> Any:
    ...
    @st.cache_data(ttl=get_settings().cache_ttl_seconds, show_spinner="Loading radar stats...")
    def _query(pids: tuple[str, ...], comp_id: int | None) -> Any:
    ...
    return _query(
        tuple(str(pid) for pid in canonical_player_ids),
        int(competition_id) if competition_id is not None else None,
    )
...
def _render_player_select_for_embeddings(
    ...
    def _query(tbl: str, tot_col: str, min_m: int, comp_id: int | None) -> Any:
        comp_filter = ""
    ...
    df = _query(table, total_col, min_matches, int(competition_id) if competition_id is not None else None)
