...
- **Liquid clustering over Z-ordering**: Mart fact tables use `liquid_clustered_by` (not `cluster_by`) for incremental, automatic data layout (40 of 42 mart fact tables; the two exceptions are the small `fct_gk_defensive_line` and `fct_gk_tracking_stats`). Liquid clustering is required for all new fact tables.
