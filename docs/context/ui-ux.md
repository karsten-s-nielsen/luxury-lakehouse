# Context — App Performance, UX Standards & Taipy UI

On-demand detail for the App/UX rules in `AGENTS.md` (root) and the Taipy rules in `hf_taipy_app/AGENTS.md`. The Taipy detail below is the full narrative moved out of the nested class-1 file. UX standards derive from CHI-AUDIT-180 and CHI-AUDIT-190 (cognitive-interface-audit v1.8.0+).
...
- **Model selectors need comparison affordance**: When adding a selector that switches between models/algorithms/views, consider the comparison workflow: side-by-side layout, delta indicators, or at minimum persist the previous selection's values visually.
...
# Taipy App — UI Architecture (full detail)
...
- **`SubView.stats` for top-tile sub-view pages**: a `SubView` may carry its own `stats: list[StatCard]` — `_build_sub_view` renders them as a top `ll-stats-bar` (the same StatCard renderer as the dashboard layout) above that view's content. When a `SubView` sets `stats` and leaves `metrics` empty, its content renders full-width (no 3fr/1fr right rail) — i.e. KPI tiles on top, full-width charts below, on a multi-view page (e.g. Goalkeeper Analytics). Sub-views with `metrics` keep the legacy 3fr/1fr grid. Same `help_text` convention applies. (`PageConfig.stats` still triggers the single-view dashboard layout; the two are independent.)
