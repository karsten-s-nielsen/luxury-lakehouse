...
# Football2Vec 360 Player Embeddings &mdash; 208-Dim Transformer + Deep Sets Vectors
...
This dataset occupies a **separate embedding space** from [Football2Vec v2 Player Embeddings](https://huggingface.co/datasets/luxury-lakehouse/football2vec-player-embeddings) (192-dim). Vectors from the two models are not directly comparable and must not be mixed in the same similarity index.
...
|----------|-------------|
| [Football2Vec 360 Model](https://huggingface.co/luxury-lakehouse/football2vec-360) | 208-dim model that generated these embeddings |
...
| [Football2Vec v2 Embeddings](https://huggingface.co/datasets/luxury-lakehouse/football2vec-player-embeddings) | 192-dim event-only embeddings with broader coverage |
| [Football2Vec v2 Model](https://huggingface.co/luxury-lakehouse/football2vec-v2) | 192-dim event-only transformer model |
