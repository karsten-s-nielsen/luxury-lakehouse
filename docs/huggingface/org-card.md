...
| [xg-v3-model-set-encoder](https://huggingface.co/luxury-lakehouse/xg-v3-model-set-encoder) | Deep Sets (Zaheer et al. 2017) + MC dropout (Gal &amp; Ghahramani 2016), canonical-SPADL **pre-shot** two-mode gate (context-aware vs tabular-only) | All-provider OOS AUC 0.742, all six providers, freeze-frame set encoder ([ADR-066](https://github.com/karsten-s-nielsen/luxury-lakehouse/blob/main/docs/superpowers/adrs/ADR-066-canonical-spadl-preshot-xg-fct-shot-xg.md)) |
...
| [obso-pausa-inputs](https://huggingface.co/datasets/luxury-lakehouse/obso-pausa-inputs) | 7 matches | ELASTIC action-frame-aligned SPADL-action inputs for OBSO/PAUSA computation |
...
| [xg-shot-data-v3](https://huggingface.co/datasets/luxury-lakehouse/xg-shot-data-v3) | ~138K shots | Tabular half of the pre-shot xG v3 training corpus &mdash; canonical-SPADL shot geometry + outcome, keyed `(match_key, action_id)`, all providers. RM-SkillCorner + GradientSports partitions in a private org-members-only companion `-restricted` ([ADR-049](https://github.com/karsten-s-nielsen/luxury-lakehouse/blob/main/docs/superpowers/adrs/ADR-049-restricted-hf-dataset-companion-repos.md) / [ADR-064](https://github.com/karsten-s-nielsen/luxury-lakehouse/blob/main/docs/superpowers/adrs/ADR-064-per-match-access-tier.md)). |
| [xg-shot-freeze-frames](https://huggingface.co/datasets/luxury-lakehouse/xg-shot-freeze-frames) | ~201K player rows | Context half of the pre-shot xG v3 corpus &mdash; per-shot freeze-frame player sets (StatsBomb-360 + tracking), joined to `xg-shot-data-v3` on `(match_key, action_id)`, canonical SPADL 105&times;68. RM-SkillCorner + GradientSports partitions in the private `-restricted` companion (ADR-049 / ADR-064). |
...
| [spadl-action-context](https://huggingface.co/datasets/luxury-lakehouse/spadl-action-context) | 100K&ndash;1M actions | Per-action tracking-derived context features (pitch control, team shape, OBSO, PAUSA, xT-GK, line-breaking, shot geometry) &mdash; one row per SPADL action for the 4 tracking providers + StatsBomb-360 (frames-required, ADR-057). GradientSports partitions in a private org-members-only `spadl-action-context-restricted` companion (ADR-049). |
