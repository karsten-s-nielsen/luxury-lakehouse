...
- **11 Streamlit dashboard pages** deployed natively on Databricks Apps, serving coaches, scouts, and analysts
- **17 synced tables** with Zero-ETL continuous sync from Gold Delta Lake to Lakebase PostgreSQL 17
- **36 PostgreSQL indexes** (32 btree + 4 HNSW vector indexes) for sub-10ms OLTP queries
- Pipeline reliability enforced through **614 unit tests** and **381 dbt data tests**
...
The [Soccer Analytics Explorer](https://huggingface.co/spaces/luxury-lakehouse/soccer-analytics-demo) is a 5-tab Gradio Space demonstrating pass quality analysis, physics-based pitch control surfaces, player similarity search, shot maps, and DEFCON defensive pressure breakdowns &mdash; all running on pre-cached Parquet data with no database dependency.
