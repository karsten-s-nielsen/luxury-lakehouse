...
**Downstream (2026-07 mart consolidation):** the redundant gold mart `fct_off_ball_xt` was retired. Off-ball-xt aggregates are consumed directly from the surviving `stg_off_ball_xt__results` staging view by `fct_physical_stats`. The bronze output schema is unchanged — provider derivation lives in staging via `match_id` prefix CASE.
