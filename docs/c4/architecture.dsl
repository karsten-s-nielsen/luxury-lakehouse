workspace "(Right! Luxury!) Lakehouse" "Serverless soccer analytics platform built on Databricks Lakebase, replacing a traditional 6-service AWS pipeline with a unified lakehouse architecture." {
    ...
    model {
        ...
        idsse = softwareSystem "IDSSE Bundesliga Dataset" "Figshare collection providing DFL position tracking XML and event XML at 25fps for 7 Bundesliga matches (CC-BY 4.0)" "External"
        ...
        hfHub = softwareSystem "HuggingFace Hub" "ML artifact registry hosting 2 models (football2vec, xG), 7 published datasets (SPADL/VAEP, Line-Breaking Passes, Player Embeddings, Pitch Control, Expected Threat, OBSO-PAUSA Inputs, OBSO-PAUSA Values), and 6-tab Gradio demo Space at luxury-lakehouse/" "External"
        hfJobs = softwareSystem "HuggingFace Jobs" "Ephemeral GPU compute containers (A10G) for ML training and batch analytics via PEP 723 UV scripts" "External"
        ...
        platform = softwareSystem "Soccer Analytics Platform" "Serverless soccer analytics platform that ingests open-source match data, transforms it through a medallion architecture, and serves interactive dashboards for coaches and analysts" {
            ...
            ingestion = container "Ingestion Workflows" "Fetches raw data from StatsBomb, Metrica, Wyscout, IDSSE, and SkillCorner, writing to the Bronze layer in Unity Catalog. Includes SPADL/VAEP pipeline reading from bronze." "Python, statsbombpy, xml.etree, kloppy, Databricks Serverless Compute" {
                ...
                idsseEventsComp = component "IDSSE Events Ingester" "Parses DFL event XML (DFL_03_02 series) from UC Volume. Extracts event type, position, player, team, and period-relative timestamps for 7 Bundesliga matches." "Python, xml.etree.ElementTree"
                elasticSyncPipeline = component "ELASTIC Sync Pipeline" "Driver-per-match sequential processing: loads tracking to pandas, calls align_events_to_frames(), writes elastic_sync_results to Bronze." "Python, analytics.elastic_sync"
                pausaPipeline = component "PAUSA Pipeline" "Reads OBSO raw scores and ELASTIC sync results, computes temporal judgment + spatial selection + PAUSA composite, writes fct_pausa_values to Gold." "Python, analytics.pausa"
                modelValidationPipeline = component "Model Validation Pipeline" "Post-dbt-build validation: PSI on xG, Wasserstein on xT/VAEP, CUSUM on line-breaking, physical bounds on tracking, writes model_validation_runs." "Python, analytics.model_validation"
            }
            analytics = container "Analytics Models" "Pure-Python analytics library providing physics-based pitch control (Spearman 2017), line-breaking pass detection (Ward clustering + straddle test), off-ball expected threat valuation, xG model inference, and data-driven xT grid computation" "Python, NumPy, SciPy" {
                ...
                elasticSyncModel = component "ELASTIC Sync" "Event-tracking frame alignment via ball acceleration and player-ball proximity features. Kim et al. (2025) ECML-PKDD." "elastic_sync.py, NumPy"
                obsoModel = component "OBSO Model" "Off-Ball Scoring Opportunity value surface: PPCF x Transition x EPV. Includes ghost trajectory generation for counterfactual analysis. Spearman 2018, Fernandez and Bornn 2018." "obso.py, NumPy, JAX"
                pausaModel = component "PAUSA Model" "Pass timing decomposition: temporal judgment (when) x spatial selection (where). Lee et al. MIT Sloan 2026." "pausa.py, NumPy"
                augmentationModel = component "Tracking Augmentation" "Physics-based position jitter (10x) composed with symmetry augmentation (8x) for 88x in-memory training data multiplication. TacticAI (Wang et al. 2024)." "augmentation.py, NumPy"
                modelValidationModel = component "Model Validation" "Drift detection: PSI, Wasserstein distance, CUSUM, KS test, physical bounds. Pure scipy/numpy." "model_validation.py, scipy.stats"
            }
            ...
            dbt = container "dbt Project" "Transforms raw Bronze data through Silver staging to Gold analytics tables, including xG features, pass metrics, player statistics, and multi-source tracking frames" "dbt-core, dbt-databricks" {
                ...
                stagingIdsse = component "IDSSE Staging" "3 views: tracking, events, elastic_sync. Transforms center-origin meters to 120x80 coordinate system, adds source_provider (idsse) and frame_rate (25)" "SQL Views, Silver Schema"
                ...
                stagingPausa = component "PAUSA Staging" "2 views: PAUSA values (dedup by pass_id), pass quality (ephemeral CTE joining dim_players). Gated by pausa_enabled toggle." "SQL Views, Silver Schema"
            ...
            syncedTables = container "Synced Tables Pipeline" "19 synced tables (16 fact, 3 dimension) replicate Gold Delta tables into Lakebase via SNAPSHOT scheduling. 38 indexes (34 btree + 4 HNSW) across fact tables for sub-100ms queries." "Lakeflow Synced Database Tables, Terraform" "Queue"
            ...
            streamlit = container "Streamlit Dashboard" "Interactive analytics dashboard deployed on HuggingFace Spaces (Docker SDK) with 12 pages covering event analysis, player comparison, player similarity search, movement analysis, defensive pressure, pass timing, and multi-source tracking visualization. Queries Lakebase PostgreSQL via OAuth." "Python, Streamlit, mplsoccer, psycopg2, HuggingFace Spaces" {
                ...
                feedbackComp = component "Feedback Helpers" "Standardized empty_select (guidance, blue), empty_result (no data, yellow), data_scope_note (coverage context), data_freshness (per-page timestamp), render_scope_label (active filter display)" "feedback.py"
                glossaryComp = component "Glossary & Onboarding" "30+ domain term definitions, per-page context filtering (PAGE_TERMS), METRIC_HELP dict for st.metric help= tooltips, Getting Started sidebar guide" "glossary.py"
                ...
                actionValuesPage = component "Player Impact Page" "3 views: Player VAEP Rankings table, Action Type Breakdown bar chart, Match Action Timeline. Filters by competition, team, player, match." "action_values.py"
                ...
                passTimingPage = component "Pass Timing Page" "PAUSA pass quality: OBSO heatmap, temporal vs spatial scatter, player rankings. Help tooltips, academic citations, LIMIT 500." "pass_timing.py"
            }
        }
        ...
        platform -> idsse "Reads pre-downloaded DFL position tracking and event XML (7 matches)" "xml.etree, UC Volume"
        platform -> skillcorner "Fetches broadcast tracking JSONL (10 matches)" "kloppy, HTTPS"
        platform -> hfHub "Publishes 2 models (football2vec, xG), 7 datasets, and 6-tab Gradio demo Space" "huggingface_hub, HTTPS"
        platform -> hfJobs "Runs GPU batch compute (OBSO, xG training)" "PEP 723 UV scripts, A10G"
        ...
        hfJobs -> hfHub "Publishes computed results" "huggingface_hub, HTTPS"
        ...
        ingestion -> idsse "Reads pre-downloaded DFL position tracking and event XML for 7 Bundesliga matches" "xml.etree, UC Volume"
        ...
        ingestion -> hfHub "Publishes 2 models (football2vec, xG) and 7 datasets via notebooks" "huggingface_hub, HTTPS"
        ...
        idsseEventsComp -> idsse "Parses DFL event XML via iterparse" "xml.etree"
        idsseEventsComp -> utilsComp "Uses Delta writer, logging, validation" ""
        elasticSyncPipeline -> catalog "Reads tracking and events from Bronze, writes elastic_sync_results" "PySpark SQL"
        elasticSyncPipeline -> analytics "Calls align_events_to_frames()" ""
        pausaPipeline -> catalog "Reads OBSO scores and sync results, writes fct_pausa_values to Gold" "PySpark SQL"
        pausaPipeline -> analytics "Calls compute_pausa_scores()" ""
        modelValidationPipeline -> catalog "Reads Gold tables, writes model_validation_runs" "PySpark SQL"
        modelValidationPipeline -> analytics "Calls PSI, Wasserstein, CUSUM validation functions" ""
        ...
        stagingIdsse -> catalog "Reads Bronze IDSSE tracking and events, writes Silver views" "Databricks SQL"
        ...
        stagingPausa -> catalog "Reads Bronze PAUSA values, writes Silver views" "Databricks SQL"
        ...
        factTables -> stagingPausa "Builds fct_pausa_values and fct_pass_timing from PAUSA staging" ""
        ...
        testSuite -> stagingPausa "Validates PAUSA value data quality" "dbt-expectations"
        ...
        appEntry -> passTimingPage "Routes to" ""
        appEntry -> configComp "Reads settings" ""
        appEntry -> glossaryComp "Renders sidebar glossary and onboarding" ""
        appEntry -> feedbackComp "Renders data freshness indicator" ""
        ...
        passTimingPage -> dbComp "Queries fct_pausa_values_synced, fct_pass_timing_synced" ""
        passTimingPage -> filtersComp "Uses match, team, player filters" ""
    ...
    views {
        ...
        dynamic platform "IngestionFlow" {
            ...
            ingestion -> idsse "Downloads DFL position and event XML for 7 Bundesliga matches"
            ingestion -> skillcorner "Downloads broadcast tracking JSONL for 10 A-League matches"
            ingestion -> catalog "Writes 14 bronze Delta tables with audit columns"
            dbt -> catalog "Transforms Bronze to Silver to Gold (381 data tests)"
            syncedTables -> catalog "Reads Gold Delta tables"
            syncedTables -> lakebase "Syncs 19 tables via Lakeflow, 38 indexes (34 btree + 4 HNSW) on partitions"
