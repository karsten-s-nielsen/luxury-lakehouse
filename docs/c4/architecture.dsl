...
workspace "Luxury Lakehouse" "Serverless soccer analytics platform on Databricks. Medallion architecture (bronze/silver/gold), dbt-managed marts, Lakebase Postgres for low-latency UI reads, Taipy HF Space for dashboards." {
    ...
    model {
        ...
        pipelinePlatform = softwareSystem "AI/ML Pipeline Platform" "47 workflow-card-registered workflows. Centralized hooks, lifecycle tracking, three-tier cost tracking." {
            ...
            ingestionPipelines = container "Compute Pipelines" "39 @workflow-decorated Databricks pipelines across 6 providers. GS bronze writer dedups duplicate (period, frameNum) records keep-first at the data-quality boundary (ADR-030). silly-kicks 4.x ET-direction derivers per provider: IDSSE reads DFL XML extraTimeFirstHalf KickOff, Metrica infers from period-3 SHOT positions, GS reads stadiumMetadata.homeTeamStartLeftExtraTime (ADR-029)." "Python, PySpark"
            ...
            actionContextHexagon = container "Action Context Hexagon" "Pure-domain enrichment for bronze.spadl_action_context (ADR-028). 5 Protocol ports + enrich_batch contract. Same code runs in the Spark applyInPandas UDF (per 250-frame batch) and in local run_work_unit on Parquet -- prod/local parity is structural. ET symmetric guard (ADR-029): MatchMeta.home_team_start_left_extratime plumbs through every convert_to_frames / convert_to_actions; lakehouse-domain _fill_possession_from_set_piece_actions synthesizes possession for SPADL set-piece restarts. Includes ghost_gk and cover_shadows detailed=True. M13 early-return short-circuits zero-owned batches before enrichment. 104-col schema incl. ghost_gk. silly-kicks 4.4: ghost-GK KDE backends scipy/vectorized/cpu-numba (ADR-035); AC-1 enrich_batch uses kde_backend='cpu-numba' — 9.6x the vectorized default and machine-precision-exact (mode/mean/spread exact, no golden re-baseline; mini-golden stays green) since ghost-GK is ~74% of the tracking chain (serverless speedup verification pending; FFT-KDE ~2000x is the eventual silly-kicks lever). DAS offside carrier-forwarding (shipped 4.2.0) is a CORRECTNESS fix that changed das_team/opponent/diff (on-ball carrier no longer mis-flagged offside) — AC-1 golden re-baselined to 4.4, ADR-036. An always-on CI gate (test_mini_golden) recomputes a 3-action slice vs a frozen mini-golden so a tracking-enrichment value shift cannot ride main uncaught (closes the #328/ADR-036 gap; the gated full AC1_E2E e2e stays the broad 97-action check). Progress/timeout visibility via exec_visibility (ADR-031); the old serverless hang is resolved on 4.x (compute now times out on ghost-GK slowness, not a hang). preflight_action_context accepts optional provider / max_units job parameters for scoped ad-hoc backfills (next-N units, one or all providers); empty defaults preserve the all-providers daily run." "Python, pandas, silly-kicks 4.4"
