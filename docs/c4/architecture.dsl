...
workspace "Luxury Lakehouse" "Serverless soccer analytics platform on Databricks. Medallion architecture (bronze/silver/gold), dbt-managed marts, Lakebase Postgres for low-latency UI reads, Taipy HF Space for dashboards." {
    ...
    model {
        ...
        pipelinePlatform = softwareSystem "AI/ML Pipeline Platform" "47 workflow-card-registered workflows. Centralized hooks, lifecycle tracking, three-tier cost tracking." {
            ...
            actionContextHexagon = container "Action Context Hexagon" "Pure-domain AC-1 enrichment to bronze.spadl_action_context (ADR-028): 5 ports + enrich_batch, same UDF in Spark (AQE-proof mapInPandas dispatch + single-pass write, ADR-045) + local run_work_unit. Time-base + id-dtype guards (ADR-040/042); xT-GK + restart-coords (ADR-048)." "Python, pandas, silly-kicks 4.22.0"
