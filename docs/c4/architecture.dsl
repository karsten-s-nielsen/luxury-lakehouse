workspace "Luxury Lakehouse" "Serverless soccer analytics platform: 35 AI/ML workflows, three-tier cost tracking, 16-page Taipy dashboard on HF Spaces, Databricks Lakebase. Evolve Engine (AlphaEvolve-style LLM-guided architecture search via OpenEvolve). Four-pillar GK evaluation (PSxG, distribution xT, collection, sweeper). Football2vec 360-enriched embeddings (144d transformer + Deep Sets). ScoutGPT decoder (256d GPT-style, player-conditioned, Hong et al. 2025). Shape graph formation detection (Sotudeh 2026), Football2vec v2 transformer with adversarial debiasing (128d). Semgrep SAST, ruff S (bandit), and import-linter boundary enforcement in CI." {
    ...
    model {
        ...
        pipelinePlatform = softwareSystem "AI/ML Pipeline Platform" "36 workflow-card-registered compute pipelines with @workflow decorators, centralized bootstrap hooks, lifecycle tracking, three-tier cost tracking, YAML manifests, Evolve Engine for LLM-guided architecture search, and import-linter boundary enforcement" {
            ...
            freshnessGate = container "Freshness Gate" "Centralized skip guard orchestrator. Calls all SkipGuard adapters (33 guards), emits SKIPPED records for idle workflows, writes dual task values (JSON FilterResult for pipelines + integer count for Terraform condition_task gates). 27 zero-cost condition_task gates prevent idle compute tasks from starting. Runs in default environment (wheel only)" "Python, PySpark, src/ingestion/freshness_gate.py"
            guardRegistry = container "Guard Registry" "SkipGuard protocol + FilterResult dataclass + find_new_ids() (Spark LEFT ANTI JOIN) + read_gate_result() (task value reader). Mandatory injection: filter_result is a required parameter on all run_pipeline() functions (no default). Pipelines cannot run their own guards. main() resolves via read_gate_result() (production) or skip_guard.check() (standalone). 6 conformance test classes (21 tests) enforce import isolation, mandatory params, no inline guards, standalone fallback, early exit structure, and early exit behavior" "Python, src/ingestion/guards.py"
        ...
        databricksWorkflows = softwareSystem "Databricks Workflows" "Scheduled DAG orchestration: 54 tasks (1 freshness gate, 27 condition_task gates, 5 ingest, 2 backfill, 14 compute, 1 HF sync, 2 360 pipeline, 1 entity resolution, 1 tracking metadata), performance-optimized mode (1-4s cold starts), daily 06:00 UTC" "External"
    ...
    views {
        ...
        dynamic pipelinePlatform "FreshnessGate" {
            ...
            freshnessGate -> guardRegistry "Loads all 29 SkipGuard adapters, calls check()"
