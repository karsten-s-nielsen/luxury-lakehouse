workspace "Luxury Lakehouse" "Serverless soccer analytics platform: 36 AI/ML workflows, three-tier cost tracking, 16-page Taipy dashboard on HF Spaces, Databricks Lakebase. Daily Databricks job (28 tasks): 26 ingestion + compute → dbt_build (python_wheel_task materializes 33 gold marts) → refresh_synced_tables (Lakebase propagation). Evolve Engine (AlphaEvolve-style LLM-guided architecture search via OpenEvolve). Four-pillar GK evaluation (PSxG, distribution xT, collection, sweeper). Football2vec 360-enriched embeddings (144d transformer + Deep Sets). ScoutGPT decoder (256d GPT-style, player-conditioned, Hong et al. 2025). Shape graph formation detection (Sotudeh 2026), Football2vec v2 transformer with adversarial debiasing (128d). SHA-256 artifact integrity verification on model loads (SEC-AUDIT ML-02). EU AI Act gap analysis in AI_GOVERNANCE.md covering 13 per-player evaluative ML systems (SEC-AUDIT REG-01), enforced by test_ai_governance_md.py; every HuggingFace model card carries an Intended-Use / Non-Use stanza and every workflow card carries a governance: YAML block. Semgrep SAST, ruff S (bandit), and import-linter boundary enforcement in CI." {
    ...
    model {
        ...
        databricksWorkflows = softwareSystem "Databricks Workflows" "Scheduled DAG orchestration: 28 tasks total: 5 ingest as DAG roots, 2 backfill, 14 compute, 1 HF sync, 2 360 pipeline, 1 entity resolution, 1 tracking metadata, dbt_build python_wheel_task (materializes 33 gold marts, depends on 9 leaves), and final refresh_synced_tables task (depends only on dbt_build — D59 collapsed 9-way fan-in to single edge). Each pipeline task runs its own skip guard at startup, performance-optimized mode (1-4s cold starts), daily 06:00 UTC" "External"
    ...
    views {
        ...
        dynamic pipelinePlatform "DailyJobHardening" {
            ...
            dbtRunner -> dbtProject "dbtRunner().invoke(['build', ...]) — 33 marts rebuilt, 591+ tests run"
            ...
            refreshSyncedTables -> databricksApi "Triggers SNAPSHOT on 34 Lakebase synced tables; polls until COMPLETE"
            autoLayout
        }
