...
workspace "Luxury Lakehouse" "Serverless soccer analytics platform on Databricks. Medallion architecture (bronze/silver/gold), dbt-managed marts, Lakebase Postgres for low-latency UI reads, Taipy HF Space for dashboards." {
    ...
    model {
        ...
        pipelinePlatform = softwareSystem "AI/ML Pipeline Platform" "47 workflow-card-registered workflows. Centralized hooks, lifecycle tracking, three-tier cost tracking." {
            ...
            ingestionPipelines = container "Compute Pipelines" "38 @workflow-decorated Databricks ingestion/compute pipelines across 6 providers. GS bronze frame dedup keep-first (ADR-030); per-provider silly-kicks 4.x ET-direction derivers (ADR-029); Metrica CSV GK identification derived empirically from period-1 positional depth (jersey-1 heuristic retired 2026-06-11). IDSSE/Sportec DFL parsing delegated to the silly-kicks parse port (ADR-055, delete-and-depend): the lakehouse no longer parses DFL XML — parse/shape lifted to silly_kicks.providers.sportec, guarded by a committed port-output golden." "Python, PySpark, silly-kicks 4.30.0"
            ...
            actionContextHexagon = container "Action Context Hexagon" "Pure-domain AC-1 enrichment to bronze.spadl_action_context (ADR-028): 5 ports + enrich_batch, same UDF in Spark (AQE-proof mapInPandas dispatch + single-pass write, ADR-045) + local run_work_unit. Two-sided time-base guards + per-unit action-completeness invariant + global M13 ownership anchors (ADR-040 am.) + id-dtype guard (ADR-042); per-provider frame batching, run-overridable (ADR-047 am. 2); xT-GK + restart-coords (ADR-048)." "Python, pandas, silly-kicks 4.30.0"
