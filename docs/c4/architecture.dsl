workspace "Luxury Lakehouse" "Serverless soccer analytics platform: 39 AI/ML workflows, three-tier cost tracking, 16-page Taipy dashboard on HF Spaces, Databricks Lakebase. Daily Databricks job (28 tasks): 26 ingestion + compute → dbt_build (python_wheel_task materializes 36 gold marts) → refresh_synced_tables (Lakebase propagation). Evolve Engine (AlphaEvolve-style LLM-guided architecture search via OpenEvolve). Four-pillar GK evaluation (PSxG, distribution xT, collection, sweeper). Football2vec 360-enriched embeddings (144d transformer + Deep Sets). ScoutGPT decoder (256d GPT-style, player-conditioned, Hong et al. 2025). Shape graph formation detection (Sotudeh 2026), Football2vec v2 transformer with adversarial debiasing (128d). SHA-256 artifact integrity verification on model loads (SEC-AUDIT ML-02). EU AI Act gap analysis in AI_GOVERNANCE.md covering 13 per-player evaluative ML systems (SEC-AUDIT REG-01), enforced by test_ai_governance_md.py; every HuggingFace model card carries an Intended-Use / Non-Use stanza and every workflow card carries a governance: YAML block. Workflow-card governance extended 2026-04-16 (ADR-003): dbt-derived outputs declare `dbt_model:` on TableRef, enforced by test_card_dbt_model_field; cost.<phase> keys must match execution.<phase> keys, enforced by test_card_cost_phase_parity; databricks_job block ordering generalised across every job in terraform/, enforced by test_workflows_tf_ordering. Semgrep SAST, ruff S (bandit), and import-linter boundary enforcement in CI." {
    ...
    model {
        ...
        taipyApp = softwareSystem "Taipy Dashboard" "Interactive soccer analytics application with 16 pages: 15 analytics + AI/ML Workflows operations dashboard. Conversion Funnel page built but disabled pending performance validation (D57/D58). Authenticated admin API (POST /api/cache/clear) for forced cache invalidation + on-demand synced table refresh, mounted on the underlying Flask layer via Gui(flask=...)" {
            ...
            templateEngine = container "Template Engine" "Three layout builders (standard, sub-view, dashboard) dispatched by build_page(). Dashboard layout: StatCard stats bar + ll-dashboard-scroll viewport container. Typed dataclasses: PageConfig (scope_dims + scope_vars), SubView, ContentBlock (table_cell_class_name for per-cell CSS, alt_var for scope-aware image alt text), ContentRow, SidebarWidget (required: bool auto-appends ' (optional)' to label; filterable enables Taipy client-side filter; searchable=True + search_var + on_search_change + search_label render a debounced input above an always-visible scrollable selector list for server-driven backend search — provisional pattern pending Path C Taipy extension replacement), Metric, Citation, StatCard (detail_html for content-provider iframes), ScopeDim (label + value_var for canonical scope line — ADR-008 Tier A canon). Scope rendering: per-dimension Taipy state vars rendered as small-caps label + italic value, CSS pseudo-element separator between dims, cloned by lightbox JS into overlay figcaption" "Python, frozen dataclasses"
        ...
        pipelinePlatform = softwareSystem "AI/ML Pipeline Platform" "39 workflow-card-registered workflows (30 @workflow-decorated Databricks pipelines plus 9 manual/data-movement cards including 3 HF Jobs publish scripts), centralized bootstrap hooks, lifecycle tracking, three-tier cost tracking, YAML manifests with dbt_model and cost/execution phase parity enforcement (ADR-003), Evolve Engine for LLM-guided architecture search, and import-linter boundary enforcement" {
            ...
            workflowCards = container "Workflow Cards" "39 YAML manifests defining inputs, outputs, deps, execution config, cost estimates, academic provenance. TableRef supports dbt_model: for dbt-derived outputs (ADR-003); wf-dbt-build enumerates all 36 gold marts. cost.<phase> keys must mirror execution.<phase> keys (enforced)" "YAML, workflow-cards/" "Database"
        ...
        dbtProject = softwareSystem "dbt Project" "Medallion transformation: 69 models (27 staging, 6 intermediate incl. int_running_score for per-event game state, 36 marts with game_state/possession columns), normalize_coordinates macro, data classification meta tags, model contracts, liquid clustering. dbt-owners-dev group owns dev_silver/dev_gold schemas + a +post-hook transfers per-object ownership back to the group on each build, allowing both developer user and ingestion SP to REPLACE objects without lockout" {
            ...
            goldModels = container "Gold Models" "32 fact tables + 4 dimension tables with enforced contracts, liquid clustering, auto-compaction. Includes 3 pre-aggregated base-case marts (fct_heatmap_agg, fct_vaep_breakdown_agg, fct_gk_actions_detail) added 2026-04-17 to replace comp-only Parallel Seq Scans on >1M-row fact tables with sub-100ms index scans" "SQL, dbt" "Database"
        }
    ...
    views {
        ...
        dynamic pipelinePlatform "DailyJobHardening" {
            ...
            dbtRunner -> dbtProject "dbtRunner().invoke(['build', ...]) — 36 marts rebuilt, 591+ tests run"
