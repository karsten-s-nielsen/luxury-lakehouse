...
- `fct_xg_predictions_v2` is retired to a back-compat surface that **projects** `fct_shot_xg` into the legacy schema. It is materialized as a **table**, not a view (C-b): `fct_xg_predictions_v2_synced` is a SNAPSHOT Lakebase synced table and a synced table must source from a Delta table — a view would strand it and break the Taipy shot-map. The synced-table source requirement is the "measured need" (V-5) for escalating to materialized; it also avoids a 2-hop bridge join on every shot-map load. The projection is provably 1:1 and covers the identical shot set as the legacy table (131,074 statsbomb+wyscout shots, verified live). The Taipy shot-map label moves `v2 Set Encoder → v3 Set Encoder` in the same change (no silent substitution — values shift v2-model → v3-model, an improvement). The old producer chain (`compute_xg_model_v2`, `bronze.xg_predictions_v2`, `stg_xg__predictions_v2`) then feeds nothing and is retired in a follow-up (the dual-model-window closure).
