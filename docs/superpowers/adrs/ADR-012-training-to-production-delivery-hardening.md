...
- **`require_mlflow_env()`** — call at the top of `main()` before any work. Raises `RuntimeError` when `MLFLOW_TRACKING_URI` / `DATABRICKS_HOST` is missing or no Databricks credential is present. The `if tracking_uri:` gate pattern is forbidden going forward — MLflow registration is mandatory. **Amended by [ADR-079](ADR-079-hf-jobs-m2m-oauth-training-auth.md): the credential may be EITHER `DATABRICKS_TOKEN` OR M2M OAuth service-principal creds (`DATABRICKS_CLIENT_ID` + `DATABRICKS_CLIENT_SECRET`) — the M2M form auto-refreshes inside the job and is required for any run longer than an OAuth access token's ~1h TTL.**
...
**Test-enforced across all Champion-registering trainers (2026-06-06).** `src/tests/test_inference_trainer_artifact_deploy.py` parametrizes over the 6 trainers that register an MLflow `@Champion` (`train_xg_v2_hf`, `train_football2vec`, `train_vaep_model_hf`, `train_football2vec_v2`, `train_football2vec_360`, `train_scoutgpt_hf`) and AST-asserts each one imports + calls `require_mlflow_env()` and `set_and_verify_mlflow_champion()`, and contains neither the silent `if tracking_uri:` gate nor a direct `set_registered_model_alias`. This generalizes the per-script regression in `test_train_xg_v2_hf.py` so the bug class cannot drift back in (it had: 4 of the 6 trainers had silently reverted to the pre-hardening pattern, surfaced + fixed in the silly-kicks 4.13 VAEP retrain). Leg #3 (`upload_weights_to_uc_volume`) is NOT universally enforced by that test — it is N/A for models whose Databricks consumer loads from MLflow `@Champion` only (e.g. VAEP's `spadl_vaep.py` self-caches run-scoped from MLflow, never reading a static UC-Volume path); enforce it per-trainer where the consumer reads a static `/Volumes/.../model_weights/<name>/` path. **Audited 2026-06-06** (silly-kicks 4.13 VAEP retrain): of the 4 trainers hardened that cycle, none require leg #3 — `football2vec_v2`, `football2vec_360`, and `scoutgpt` all generate + publish their embeddings *during training* (the Databricks product is the embeddings table / HF dataset, not a re-loaded weight file), and VAEP self-caches from MLflow. The only model whose Databricks consumer reads a static UC-Volume weight file is football2vec **v1** (`player_embeddings_v1.py` → `/Volumes/.../model_weights/football2vec/player2vec.model`), whose trainer already writes it.
