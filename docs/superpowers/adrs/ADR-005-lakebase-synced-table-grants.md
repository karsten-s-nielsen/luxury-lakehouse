...
**2026-07-09 (SP resolution decoupled from local terraform):** `run_lakebase_grants.py` resolved the Taipy SP `application_id` via `terraform output -raw hf_app_sp_application_id`, which requires an initialized local terraform (provider plugins cached + state access). Terraform runs only in CI, so a **local** `manage_space.py deploy` hit the grants gate, shelled out to `terraform output`, failed with "provider plugins not installed", and mis-reported it as "SP missing SELECT" — a false negative that blocked a healthy staging deploy. The default resolution now reads the source of truth — the live Databricks workspace — via the **SDK** (`service_principals.list(filter='displayName eq "luxury-lakehouse-hf-app-v2-dev"')`), using the `DATABRICKS_HOST`/`TOKEN` the deploy already needs; `--sp-uuid` / `LAKEBASE_TAIPY_SP_APP_ID` remain explicit overrides. CI is unaffected — `lakebase-grants.yml` already passes `--sp-uuid` from `vars.HF_APP_SP_APPLICATION_ID` and never reached the terraform fallback. The display-name constant is anti-drift-tested against the terraform module (`test_lakebase_grants_sp_resolution.py`). This is *more* drift-safe than `terraform output` (reads the real workspace, not terraform's cached view) and needs no local terraform.
