...
## Amendment (2026-06-20, TF-23 / ADR-053 amendment): AC SC/metrica converters retired upstream

On the **AC path**, `_bronze_{skillcorner,metrica}_to_frames` and the lakehouse `_SKILLCORNER_PERIOD_START_SECONDS` copy in `convert.py` are **deleted** — SC/metrica frames are now produced by the silly-kicks `tracking.{skillcorner,metrica}.convert_to_frames` builders (via `analytics.action_context.sk_frame_adapters`), and the adapter **overwrites** the builder clock with the dispatch-layer period-relative timestamp (B′ `(frame_id, period_id)` map-join). The dispatch re-bases above are **unchanged** and remain the single owner of each provider's re-base; the SC offset is now imported from `silly_kicks.spadl.skillcorner._PERIOD_START_SECONDS` (identical values, guarded by `test_skillcorner_dispatch_time_base`). The retired `test_skillcorner_frame_time_base` (which asserted the deleted AC converter's internal SC re-base) is superseded by the adapter clock-overwrite tests in `test_sk_frame_adapters` / `test_sk_builder_migration_gate`. The **legacy `tracking_context.py`** converter copies + their `_SKILLCORNER_PERIOD_START_SECONDS` are untouched, so the former AC↔legacy drift-identity guard in `test_convert_drift.py` no longer applies (only the legacy copy remains; replaced by a structural "AC builders deleted" guard).
