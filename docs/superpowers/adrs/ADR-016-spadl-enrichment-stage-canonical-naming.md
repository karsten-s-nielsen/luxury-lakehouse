...
| Legacy native ID inside ADR-011 dual-column window | `<entity>_id` (legacy BIGINT/INT): `match_id`, `team_id`, `competition_id`, `season_id`, `player_id` | StatsBomb / Wyscout: numeric native ID. IDSSE / Metrica: NULL except `match_id` / `game_id` which are deterministically hashed via `hash_native_id_to_bigint` so `applyInPandas(groupBy match_id)` continues to dispatch per-match groups. |
