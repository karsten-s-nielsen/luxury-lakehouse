...
**Design:** add a `statsbomb` branch to `compute_shot_freeze_frames` (`ingestion.shot_freeze_frames`) that, per match, runs **two DISTINCT, separately-tested steps** — coordinate *conversion* first, then *orientation*. Conflating them is how the y-flip gets lost, so they are explicitly separate:
