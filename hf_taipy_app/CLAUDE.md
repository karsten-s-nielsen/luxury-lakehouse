...
- **Server-driven autocomplete (`SidebarWidget.searchable=True`)**: Dropdowns whose LOV would exceed ~200 items must use `searchable=True` + `search_var` + `on_search_change` instead of the client-side `filterable=True` flag. The template renders a debounced `|input|` (300ms default) above an always-visible scrollable selector list (the `|dropdown|` flag is intentionally omitted so results update live without a click). The search-change callback hits a `filters.search_*` function (SQL-backed) or filters an in-memory map (for DV / TAC where the candidate set is already loaded for the page's primary use). Every callback must set the LOV to `[..., filters.NO_MATCHES_SENTINEL]` when the match list is empty — Taipy's state diff treats `state.lov = []` as a no-op, so the bare empty list leaves a stale LOV visible. **Known limitation (2026-04-18):** the always-visible list consumes ~220px of sidebar height even when the user isn't searching. Path C in `docs/ui-cycles/ui-consistency-roadmap.md` plans replacement with a Taipy GUI extension wrapping MUI Autocomplete to match the WAI-ARIA APG combobox-with-list-autocomplete pattern (list visible only when focused or has content). Do NOT extend `searchable=True` to new dropdowns until Path C lands — it's a provisional UX.
