...
if __name__ == "__main__":
    ...
    # Source lives in static/lightbox.js for cleaner code separation (CHI audit #5/#23).
    # Injected inline via after_request because Taipy overrides Flask's static routing
    # and Taipy markdown strips <script> tags from page content.
    _lightbox_js_path = Path(__file__).resolve().parent.parent / "static" / "lightbox.js"
    _LIGHTBOX_TAG = f"<script>{_lightbox_js_path.read_text(encoding='utf-8')}</script>"
    ...
    # https://fonts.gstatic.com: Material Symbols font loaded via @font-face in style_v2.css.
    _CSP = (
        "default-src 'self'; "
