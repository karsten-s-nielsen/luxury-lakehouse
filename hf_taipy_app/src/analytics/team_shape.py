...
class TeamShapeResult:
    ...
    hull_vertices_x: tuple[float, ...] | None = None  # Convex hull vertex x-coords (plotting order)
    hull_vertices_y: tuple[float, ...] | None = None  # Convex hull vertex y-coords (plotting order)
    cluster_labels: tuple[int, ...] | None = None  # Ward cluster assignment per player
...
def compute_team_shape(
    ...
    # Convex hull area + vertices for rendering
    hull_vx: tuple[float, ...] | None = None
    hull_vy: tuple[float, ...] | None = None
    try:
        points = np.column_stack((px, py))
        hull = ConvexHull(points)
        hull_area = float(hull.volume)  # 2-D: volume = area
        # Extract vertices in plotting order (closed polygon)
        verts = hull.vertices.tolist()
        verts.append(verts[0])  # close the polygon
        hull_vx = tuple(float(px[v]) for v in verts)
        hull_vy = tuple(float(py[v]) for v in verts)
    except QhullError:
        hull_area = 0.0

    # Ward clustering along the attacking axis (x) for defensive lines
    n_clusters = min(params.n_defensive_lines, n)
    cluster_labels_tuple: tuple[int, ...] | None = None
    if n_clusters < 2:
        ...
        cluster_labels_tuple = tuple(int(lbl) for lbl in labels)
    ...
    return TeamShapeResult(
        ...
        hull_vertices_x=hull_vx,
        hull_vertices_y=hull_vy,
        cluster_labels=cluster_labels_tuple,
    )
