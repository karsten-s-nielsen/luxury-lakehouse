...
@ttl_cache()
def _fetch_shots(competition_id: int, team_id: int | None, player_id: int | None) -> pd.DataFrame:
...
@ttl_cache()
def _fetch_xg_predictions(competition_id: int) -> pd.DataFrame:
...
def _join_xg_predictions(shots: pd.DataFrame, competition_id: int) -> tuple[pd.DataFrame, bool]:
    ...
    xg_preds = _fetch_xg_predictions(competition_id)
    if xg_preds.empty:
        return shots, False
...
def sm_refresh(state: Any) -> None:
    ...
    # Fetch shots
    shots = _fetch_shots(comp_id, team_id, player_id)
    if shots.empty:
