...
|----------|----------|--------------------|
| **Counterfactual Pitch Control Substitution** | Trajectory normalization (how to map Player A's movement patterns into Player B's positional role — no published method), evaluation metric (total control area? xT-weighted? no consensus), match volume (20 tracking matches too few, need 50+), player ID bridge (tracking IDs not linked to `dim_players.canonical_player_id`) | Two open research questions (trajectory normalization, evaluation metric) plus data volume blocker. The pitch control model (`pitch_control.py`, Spearman 2017) already supports arbitrary player inputs — the math is ready, the methodology isn't. Becomes feasible when own-footage pipeline delivers 50+ player-identified tracking matches. Source: [adversarial-training.md](docs/research/adversarial-training.md) |
| **State-space model encoder for Football2Vec / ScoutGPT** (Mamba-2 / S4D / S5) | No sports-specific SSM-for-sequences literature, uncertain win vs transformer (EV1 found flat landscape on Football2Vec stage-1 — architecture may not be the bottleneck), CUDA-kernel dep (`mamba-ssm`) complicates the HF Jobs wheel path, interpretability regression (no attention weights). Different use case from SportMamba above, which is video-tracking. | Speculative research. EV1 (2026-04-18) found the transformer encoder delivers a parsimony gain (−34% params at same accuracy) so it isn't obviously wrong. SSMs' theoretical wins (O(N) inference, long-sequence scaling, local-context bias) match the SPADL sequence shape but no empirical motivation exists yet. Revisit if (1) EV2 stage-2 adversarial search also finds the transformer flat, (2) Mamba-2 wins on another soccer sequence benchmark, or (3) we need sub-ms inference latency per sequence. Implementation estimate: ~1 focused day for a drop-in `Football2VecSSMEncoder` sharing the current encoder API. |
