...
- **Mandatory guard injection**: Every pipeline's `run_pipeline()` receives `FilterResult` as a **required** parameter (no default). Pipelines cannot run their own guards — `find_new_ids()` is prohibited outside guard classes, enforced by `TestNoInlineGuardInPipeline`. `main()` resolves the result: `read_gate_result()` in production (Databricks task values), `skip_guard.check()` as standalone fallback. The freshness gate writes dual task values per guard: JSON `FilterResult` (for pipeline consumption) and integer count (for Terraform `condition_task` gates). 33 guards registered in `_GUARD_MODULES`, 27 Terraform `condition_task` gates prevent idle tasks from starting. Guard conformance enforced by 6 test classes (21 tests) in `test_guard_conformance.py`: import isolation, mandatory params, no inline guards, standalone fallback, early exit structure, early exit behavior.
