...
- **Mandatory guard injection**: Every pipeline's `run_pipeline()` receives `FilterResult` as a **required** parameter (no default). Pipelines cannot run their own guards — `find_new_ids()` is prohibited outside guard classes, enforced by `TestNoInlineGuardInPipeline`. `main()` resolves the result: `read_gate_result()` in production (Databricks task values), `skip_guard.check()` as standalone fallback. The freshness gate writes dual task values per guard: JSON `FilterResult` (for pipeline consumption via `read_gate_result()`) and integer count (for future Terraform `condition_task` gates — D40c). 33 guards registered in `_GUARD_MODULES`. Guard conformance enforced by 11 test classes (42 tests) in `test_guard_conformance.py` and `test_freshness_gate.py`: import isolation (AST + runtime transitive), mandatory params, no inline guards, standalone fallback, early exit structure, early exit behavior, exception propagation (AST + behavioral), count/ID consistency, cost/time capture, workflow ID consistency, task value propagation. Freshness gate parallelized with `ThreadPoolExecutor(max_workers=4)`.
