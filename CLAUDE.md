...
- **Incremental skip guards**: Every compute pipeline exposes a `SkipGuard` adapter (in `src/ingestion/guards.py` registry) whose `check()` returns a `FilterResult(workflow_id, count, chunks, metadata)`. The freshness gate task calls all guards centrally and writes results as Databricks task values. Pipelines also retain an inline fallback guard for standalone execution. Pattern: `existing = {str(row["match_id"]) for row in spark.table(results).select("match_id").distinct().collect()}`. The `str()` normalization is critical — Spark returns `int`, Delta stores `string`.
