...
- **Incremental skip guards**: Every compute pipeline exposes a `SkipGuard` adapter (in `src/ingestion/guards.py` registry) whose `check()` returns a `FilterResult(workflow_id, count, chunks, metadata)`. The freshness gate task calls all guards centrally and writes results as Databricks task values. Pipelines read gate results via `read_gate_result()` in their `main()` function and pass them to `run_pipeline(filter_result=...)`. Inline fallback guards for standalone execution use `find_new_ids()`. Pattern: `from ingestion.guards import find_new_ids; new_ids = find_new_ids(spark, source_table, results_table)`. The `find_new_ids()` helper uses Spark LEFT ANTI JOIN with `cast("string")` normalization — pushes the set difference to executors instead of collecting all IDs to the driver. Guard conformance is enforced by `test_guard_conformance.py` (auto-discovers all guards from `_GUARD_MODULES`).
