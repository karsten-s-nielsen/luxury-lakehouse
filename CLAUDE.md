...
- **dbt-derived outputs in workflow cards**: Any `outputs.tables` entry whose table is produced by a dbt model (not by a Python execution phase on the same card) MUST declare `dbt_model: <model_name>` on its `TableRef`, where `<model_name>` matches a `.sql` file under `dbt_project/models/**/`. Enforced by `test_card_dbt_model_field`. `wf-dbt-build.yaml` enumerates all 36 gold-layer mart models with `dbt_model:` set on each; `wf-goalkeeper.yaml` declares `dbt_model: fct_goalkeeper_stats` because the table is produced by `dbt build`, not by a standalone Python entry point on that card.
