...
- **Lakebase grants survive synced table recreation**: Use `ALTER DEFAULT PRIVILEGES FOR ROLE databricks_superuser IN SCHEMA <name> GRANT SELECT ON TABLES TO <sp_uuid>`. dbt `table` materialization drops + recreates the Delta table; the Lakebase sync recreates the PostgreSQL synced table; the new table is owned by `databricks_superuser` so default privileges scoped to the user running the grants script do NOT apply. The `FOR ROLE databricks_superuser` clause makes future synced tables auto-grant. Already wired into `scripts/lakebase_grants.sql` and `scripts/run_lakebase_grants.py`.
