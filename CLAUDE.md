...
- **Action-context frame orientation = flag-free geometric net** ([ADR-053](docs/superpowers/adrs/ADR-053-silly-kicks-4-27-0-geometric-frame-ltr-net.md)): `analytics.action_context.pipeline.correct_frames_to_home_ltr` is the orientation authority for the AC tracking path — applied at the `_convert_tracking_batch` dispatch tail for ALL four tracking providers. It reads direction from goalkeeper geometry (home GK must sit at LOW x in home-LTR), per-period point-reflects mis-oriented periods (`x→105−x`, `y→68−y`, `vx→−vx`, `vy→−vy`; speed invariant), populates `team_attacking_direction` only when the builder left it null, and logs every flip. Do NOT switch this path to silly-kicks `orient_frames_to_ltr` (flag-driven) — the AC driver DEFAULTS `home_start_left=True` for metrica/skillcorner (never derived) and GS extra-time tracking is per-match end-flipped vs its events (see ADR-053 Context), so a flag can't orient reliably. The legacy `tracking_context.py`/`fct_tracking_context` path is NOT yet covered — wire equivalent orientation there before recomputing it. Regression: `src/tests/action_context/test_frame_ltr_correction.py` (unit) + `test_frame_orientation_golden.py` (cross-provider golden — recomputes a real idsse/skillcorner/GS-extra-time slice and asserts home-GK-low; closes the idsse-only golden gap).
