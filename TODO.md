...
**Last updated**: 2026-04-09 (Cycle 5 Phase 3: D51 warm-tier enrichment, D52 gate removal / guard-as-wrapper, M2 OAuth M2M, TF-SP HF-app service principal)
...
|---|------|------|--------|-------|
| D53 | Wheel Version Management Procedure | Wicked | Kirk/Voss audit session + Evolve session (2026-04-09) | **Problem:** No single source of truth or coordinated procedure for wheel version bumps. The version is defined in `pyproject.toml` but consumers discover it independently: `manage_space.py` globs `dist/luxury_lakehouse-*.whl` (picks up stale + current), the Dockerfile `COPY dist/luxury_lakehouse-*.whl` installs all matches (pip conflict), 15 HF Jobs PEP 723 scripts hardcode `luxury_lakehouse-0.1.0` in their dependency URL, and `src/evolve/backends/hf_jobs.py` does the same. **Evidence:** Three independent signals in 48 hours: (1) staging Space build broken by stale 0.1.0 wheel alongside new 0.3.0, (2) workflow session struggled to confirm correct version deployed, (3) Evolve session flagged hardcoded 0.1.0 references as future task. **Affected files:** `scripts/manage_space.py` (glob logic), `hf_taipy_app/Dockerfile` (COPY glob), 15 `scripts/*_hf.py` PEP 723 scripts (hardcoded URL), `src/evolve/backends/hf_jobs.py` (hardcoded URL), `dist/` (stale wheels accumulate). **Fix approach:** (1) Read version from `pyproject.toml` at build/deploy time — `manage_space.py` should `importlib.metadata.version("luxury-lakehouse")` or parse `pyproject.toml` to select only the matching wheel, (2) CI step to update PEP 723 script URLs on version bump (or use a `latest` redirect on HF Hub `build-artifacts`), (3) `manage_space.py` `_bundle_wheel` should delete stale wheels from `dist/` before copying (or copy only the latest), (4) document the version bump checklist: bump `pyproject.toml` → `uv build` → upload wheel to HF Hub → update PEP 723 scripts → deploy Spaces. **Immediate fix (done in Kirk/Voss cycle):** Delete stale `dist/luxury_lakehouse-0.1.0*` locally, re-deploy staging. |
| D54 | Taipy Workflows page — surface enriched cost columns | Dunkin' | D51 follow-up (Cycle 5 Phase 3) | Wire `duration_seconds`, `entity_count`, `row_count`, `pipeline_state` from enriched `fct_workflow_costs` into AI/ML Workflows page stat cards and/or detail drilldown (D35). Columns available in cold tier after D51 LEFT JOIN. Depends on D51 (complete). |
...
| M1 | ~~Rotate Databricks PAT for HF Spaces~~ | ~~Dunkin'~~ | ~~HF-MIGRATION~~ | **Superseded by M2.** OAuth M2M credentials deployed (Cycle 5 Phase 3) — no expiring PAT to rotate. Remove `DATABRICKS_TOKEN` from both Spaces after M2 deployment verified. |
...
| M2 | Deploy OAuth M2M credentials to HF Spaces | Dunkin' | SEC-AUDIT-200 #2 | **Terraform + code complete (Cycle 5 Phase 3).** HF-app SP added to Terraform, UC grants (gold + observability) wired. **Remaining (manual):** (1) `terraform import` the existing SP, (2) `terraform apply`, (3) user generates OAuth client secret, (4) deploy staging with OAuth, (5) verify, (6) deploy production, (7) remove `DATABRICKS_TOKEN` PAT from both Spaces. |
