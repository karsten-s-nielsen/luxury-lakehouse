...
**Last updated**: 2026-04-09 (D53 wheel version management, SEC3 SHA-256 pinning infrastructure)
...
|---|------|------|--------|-------|
| D54 | Taipy Workflows page — surface enriched cost columns | Dunkin' | D51 follow-up (Cycle 5 Phase 3) | Wire `duration_seconds`, `entity_count`, `row_count`, `pipeline_state` from enriched `fct_workflow_costs` into AI/ML Workflows page stat cards and/or detail drilldown (D35). Columns available in cold tier after D51 LEFT JOIN. **Blocked on D55** — enrichment columns are NULL until warm-tier join keys are fixed. |
| D55 | Fix warm-tier join keys — `job_run_id`/`task_key` NULL on serverless | Wicked | D51 E2E verification (Cycle 5 Phase 3) | **Problem:** `CostEstimateHook.__init__` reads `spark.conf.get("spark.databricks.job.runId")` and `spark.conf.get("spark.databricks.task.key")` but both return `None` on serverless. All warm-tier rows in `workflow_cost_live` have NULL `job_run_id` and `task_key`. The D51 LEFT JOIN in `fct_workflow_costs` matches on these columns, so enrichment columns (`duration_seconds`, `entity_count`, `row_count`, `pipeline_state`, `estimated_cost_usd`) are always NULL. **Evidence:** 15 warm-tier rows all have `job_run_id=None, task_key=None`. Cold tier has 120 rows for 2026-04-09 but zero join matches. **Investigation needed:** (1) Verify correct Spark conf key names for serverless (`spark.databricks.job.runId` may be classic-only), (2) check if `dbutils.jobs.taskValues` context or environment variables expose the job/task metadata on serverless, (3) check Databricks SDK `WorkspaceClient` for runtime context API. **Blocks:** D54 (Taipy wiring depends on populated enrichment columns). |
