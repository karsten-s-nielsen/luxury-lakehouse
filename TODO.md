...
| DOC1 | ARCHITECTURE.md Comprehensive Update | Monstah | Final review audit (session 31) | ARCHITECTURE.md has **31 findings** of stale content accumulated over multiple cycles. The document was last substantively updated 2026-04-03. **Stale numbers (14):** Synced tables count 28→34 (6 tables missing from table + wrong PKs for 4 entries), PG indexes 44+4→50+6=56, Taipy pages table 14→16 (Goalkeeper Analytics + Tactical Positions missing), dbt mart models 25+3→29+4=33, dbt model contracts 17→32, liquid clustering usage 14→24, test modules 55→66, scripts 28+3→31+6, ingestion modules listed incomplete, dataset-cards 13→16. **Missing sections (3):** Evolve Engine (Level 2 code evolution, 5 backends, ScoutGPT target, ADR-001) has zero documentation. Freshness Gate system (`find_new_ids`, `read_gate_result`, `_GUARD_MODULES`, conformance tests) has zero documentation. `workflow-cards/` directory (36 YAML manifests) absent from repo structure tree. **Stale content (2):** Decision #6 "No Kloppy" contradicted by SkillCorner integration. Pipeline diagram missing 8 tasks. Gold/Silver model listings incomplete. **Why separate TODO:** 31 fixes spanning every section risk inaccuracy if rushed. Needs a dedicated pass where ARCHITECTURE.md is the sole focus, cross-checked section-by-section against current codebase state. |
