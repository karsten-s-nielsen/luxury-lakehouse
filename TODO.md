...
**Last updated**: 2026-04-07 (Evolve Level 2 implemented: code evolution with AST allowlist + subprocess isolation, defense-in-depth security via ADR-001, --code-evolution CLI flag, hybrid_gated_attention L2 seed, 81 tests)
...
| D40a | Selective Fan-Out Activation (`for_each_task`) | Wicked | D40 follow-up | The freshness gate computes `FilterResult.chunks` for pitch control (2 matches/chunk) and off-ball xT (2 matches/chunk), but Terraform does not yet wire `for_each_task`. **Steps:** (1) Monitor post-D40 per-task runtimes from `system.lakeflow.job_task_run_timeline`, (2) verify `for_each_task` Terraform syntax on Databricks provider, (3) wire fan-out for `compute_pitch_control` and `compute_off_ball_xt` — each iteration receives a chunk of match IDs from the gate's task values, (4) test with a manual job run. Respo.Vision will require fan-out from day one when data arrives. |
