...
**Last updated**: 2026-04-08 (Guard plumbing cycle: mandatory `FilterResult` injection across all pipelines, Terraform `condition_task` gates, `entity_count` observability, 6 conformance test classes)
...
|---|------|------|--------|-------|
| D40c | Terraform `condition_task` Gates (Blocked) | Wicked | D40 follow-up | Dynamic value references (`{{tasks.X.values.Y}}`) are rejected by the Databricks Jobs API in `condition_task` operands. The freshness gate already writes dual task values (JSON + integer count) and the code-level skip (`WorkflowSkippedError`) prevents redundant work. Condition gates would additionally prevent cluster spin-up. **Steps:** (1) Investigate correct `condition_task` syntax — may need a different reference format or lightweight Python gate tasks instead, (2) test with a manual job run, (3) wire all 27 gates. The Terraform module has a TODO(D40c) comment marking where gates should go. |
| D46 | Orphaned Workflows + ID Mismatch Cleanup | Dunkin' | Guard plumbing audit (session 33) | Two orphaned `@workflow`-decorated functions not connected to the pipeline: `wf-export-shots` (`export_shots_on_target.py`) and `wf-scoutgpt-export` (`export_scoutgpt_training_data.py`) — both have entry points in `pyproject.toml` but no Terraform task and no guard. Either remove the `@workflow` decorator (if they're standalone scripts) or add guards and Terraform tasks. Also: `wf-shape-graphs` decorator ID vs `wf-formations-sg` guard ID mismatch in `formations_shape_graph.py` — cosmetic but confusing. |
| D47 | Promote Stub Guards to Real Guards | Wicked | Guard plumbing audit (session 33) | 5 workflows have stub guards (always return count=1): `wf-statsbomb`, `wf-hf-sync`, `wf-model-validation`, `wf-football2vec-v2`, `wf-import-obso`/`wf-import-psxg`/`wf-import-space-creation`. Their code-level guard always returns count=1, so the pipeline never skips. Replace with real freshness checks: StatsBomb (new competitions/matches from API), model_validation (new model versions to validate), imports (check HF Hub artifact checksums before downloading), hf_sync (aggregate sub-operation guard results), football2vec-v2 (check if embeddings exist for current dataset). |
...
| D40e | Guard Parallelization (ThreadPoolExecutor) | Wicked | D40 follow-up | The freshness gate runs 33 guards sequentially. Guards are independent Spark SQL queries that could run concurrently. **Steps:** (1) Replace sequential loop in `freshness_gate.py:run_gate()` with `ThreadPoolExecutor`, (2) tune concurrency (start with `max_workers=4` to avoid Spark driver contention), (3) benchmark wall-clock improvement, (4) handle per-guard exceptions safely in threaded context. |
