...
| D42 | Evolve Engine Level 2 — Code Evolution | Wicked | [evolve-engine-design.md](docs/superpowers/specs/2026-04-04-evolve-engine-design.md) | Upgrade from config-only mutation (Level 1) to code evolution: LLM generates `custom_embed()` functions that are monkey-patched onto `ScoutGPTDecoder._embed()` before training. The evaluator already imports candidate modules dynamically — Level 2 adds a check for callable attributes and patches them onto the model class. Enables the LLM to invent entirely novel conditioning mechanisms beyond the 4 pre-implemented types. Includes: (a) evaluator code-patching logic with safety validation, (b) prompt engineering for OpenEvolve to generate valid PyTorch code, (c) sandbox constraints on generated code (no imports, no I/O, tensor-only ops), (d) seed programs with example `custom_embed()` to bootstrap LLM generation. Pre-req: Level 1 seed baselines complete + first evolution loop results analyzed |
