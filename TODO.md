...
| D47 | Promote Stub Guards to Real Guards | Wicked | Guard plumbing audit (session 33) | 5 workflows have stub guards (always return count=1): `wf-statsbomb`, `wf-hf-sync`, `wf-model-validation`, `wf-football2vec-v2`, `wf-import-obso`/`wf-import-psxg`/`wf-import-space-creation`. Their Terraform condition_task gates evaluate count > 0 but never skip (count is always 1). Replace with real freshness checks: StatsBomb (new competitions/matches from API), model_validation (new model versions to validate), imports (check HF Hub artifact checksums before downloading), hf_sync (aggregate sub-operation guard results), football2vec-v2 (check if embeddings exist for current dataset). |
