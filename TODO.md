...
**Last updated**: 2026-04-17 (SEC4 cycle — CI SP workspace-admins-group membership removed; Lakebase project `databricks_permissions` added with CI SP `CAN_MANAGE`; `account_admin` accepted as floor per ADR-006 with 4 cited empirical probe responses; orphan PG role `be66af99-...` dropped from Lakebase; 7-test regression guard in `src/tests/test_sec4_ci_sp_job_owner.py` + 1-test orphan guard. SEC-AUDIT-v1.12.0 INF-01 (CWE-250) CLOSED.)
