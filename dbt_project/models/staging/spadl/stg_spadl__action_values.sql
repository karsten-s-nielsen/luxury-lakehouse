{#
    PR-LL1 chicken-and-egg bridge: ensure the 4 statsbomb_* columns exist on
    the bronze ``vaep_action_values`` Delta table BEFORE this view's SELECT
    references them. Spark Delta supports ``ADD COLUMN IF NOT EXISTS``
    (DBR 13.3+; we run DBR 14+), so each statement is idempotent — no-op
    when the column already exists. Becomes dormant once the Python writer
    (``src/ingestion/spadl_conversion.py``) has run on the live pipeline
    and ``mergeSchema=true`` has propagated the columns naturally.

    NOTE: ``adapter.get_columns_in_relation``-based introspection inside
    a ``pre_hook=...`` macro call does NOT work — dbt evaluates ``config()``
    properties at parse time (``execute=False``), so introspection returns
    nothing and the pre_hook becomes empty. Static raw-SQL ALTERs are the
    correct pattern here.
#}
{{ config(
    pre_hook=[
        "ALTER TABLE " ~ source('spadl', 'vaep_action_values') ~ " ADD COLUMN IF NOT EXISTS statsbomb_possession_id BIGINT",
        "ALTER TABLE " ~ source('spadl', 'vaep_action_values') ~ " ADD COLUMN IF NOT EXISTS statsbomb_possession_team_id BIGINT",
        "ALTER TABLE " ~ source('spadl', 'vaep_action_values') ~ " ADD COLUMN IF NOT EXISTS statsbomb_play_pattern STRING",
        "ALTER TABLE " ~ source('spadl', 'vaep_action_values') ~ " ADD COLUMN IF NOT EXISTS statsbomb_under_pressure BOOLEAN",
    ]
) }}
