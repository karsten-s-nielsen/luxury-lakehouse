...
def publish_to_hf_hub(df: pd.DataFrame, hf_token: str, *, repo_id: str = DATASET_REPO, private: bool = False) -> str:
    ...
    Args:
        df: Shot DataFrame to publish (may be empty — see the sweep-only note below).
...
def main() -> None:
    ...
    public_df, restricted_df = split_restricted(shots_df, column="access_tier")

    # Fail-closed leak guard on the PUBLIC frame BEFORE upload — needs access_tier present.
    assert_no_private_leak(public_df, publisher="publish_xg_shot_data_v3_hf")
