...
# dependencies = [
#     "luxury-lakehouse @ https://huggingface.co/luxury-lakehouse/build-artifacts/resolve/main/luxury_lakehouse-0.5.82-py3-none-any.whl",
#     "numpy>=1.24",
#     "pandas>=2.0",
#     "pyarrow>=14.0",
#     "requests>=2.31",
...
def publish_to_hf_hub(df: pd.DataFrame, hf_token: str, *, repo_id: str = DATASET_REPO, private: bool = False) -> str:
    ...
    Args:
        df: Freeze-frame DataFrame to publish (may be empty — see the sweep-only note below).
        hf_token: HuggingFace API token.
        repo_id: Target dataset repo (default: the public DATASET_REPO; the restricted
            companion passes RESTRICTED_DATASET_REPO).
        private: Create the repo private (org-members only) if it does not exist yet. Does
            NOT flip an existing repo's visibility.
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for source, sub_df in df.groupby("data_source"):
            ...
            sub_df.to_parquet(out_path, index=False, engine="pyarrow")
            logger.info(
                "Wrote %s: %s rows -> %s (%s bytes)",
                source,
                f"{len(sub_df):,}",
                out_path,
...
def main() -> None:
    ...
    public_df, restricted_df = split_restricted(frames_df, column="access_tier")

    # Fail-closed leak guard on the PUBLIC frame BEFORE upload — needs access_tier present.
    assert_no_private_leak(public_df, publisher="publish_shot_freeze_frames_hf")
