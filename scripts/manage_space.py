...
def _requirements_path() -> Path:
    """The compiled pin set that ``upload_folder`` ships. Indirected so tests can relocate it.

    ONE construction site for this path. It was previously rebuilt independently inside
    ``_compile_requirements`` (twice), which is the same cross-cutting-concern-without-a-single-site
    shape ADR-075 exists to prevent — and would have been reintroduced by the very commit whose ADR
    cites it.
    """
...
def _prepare_requirements(*, compile_it: bool, expect_sha256: str | None) -> str:
    ...
    Two call sites to ``_compile_requirements()`` inside the upload path meant ``deploy staging``
    and ``deploy production`` resolved independently, so the artifact validated on staging was
    never the artifact shipped to production. Everything now routes through here, and the returned
    digest is what makes "same artifact" checkable rather than assumed.

    The digest must come from the pre-deploy manifest that VALIDATED the file, never be recomputed
    from the file at deploy time: recomputing asserts ``sha256(file) == sha256(file)`` in the one
    mode that never rewrites it — a comparison that cannot fail. See ADR-076.
    """
    if compile_it:
...
def _compile_requirements() -> str:
    ...
    Returns the sha256 of what it wrote, giving the deploy log a stable artifact identity — which
    is what makes "same pin set" checkable after the fact.

    Call it through ``_prepare_requirements`` only.
    """
...
def _dry_run(folder: Path, repo_id: str, api: HfApi, *, compile_it: bool, expect_sha256: str | None) -> None:
    """Preview what would be uploaded and deleted without making changes."""
    _prepare_requirements(compile_it=compile_it, expect_sha256=expect_sha256)
...
def _deploy(
    ...
    *,
    ...
    digest = _prepare_requirements(compile_it=compile_it, expect_sha256=expect_sha256)
    logger.info("Shipping requirements artifact sha256=%s", digest)
    cards_dst = _bundle_workflow_cards()
    wheel_dst = _bundle_wheel()
    try:
...
def _deploy_command(
    ...
    *,
    ...
    if dry_run:
        _dry_run(folder, repo_id, api, compile_it=compile_it, expect_sha256=expect_sha256)
    else:
        _deploy(
            folder,
            repo_id,
...
def main(argv: list[str] | None = None) -> int:
    ...
    ``argv=None`` preserves every existing call: ``parse_args(None)`` reads ``sys.argv`` exactly as
    before, so the ``__main__`` entry point and any console-script registration are unaffected.
    """
    parser = _build_parser()
    args = parser.parse_args(argv)

    # Both checks fire BEFORE `repo_id = TARGETS[...]` and `HfApi()`, so no Space object is
    # constructed on the failing path.
    ...
    if args.command == "deploy" and args.target == "production" and not args.no_compile:
        # Structural where it matters, following the _require_force_for_production convention:
        # production gets the gate, staging keeps the ergonomic path where the artifact is
        # PRODUCED (forcing the flags there would leave nowhere to produce one).
        parser.error(
