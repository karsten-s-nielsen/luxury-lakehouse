...
| **ML Artifacts** | [Hugging Face Hub](https://huggingface.co/luxury-lakehouse) | Publish [17 models](https://huggingface.co/luxury-lakehouse) + [19 datasets](https://huggingface.co/luxury-lakehouse), GPU training on HF Jobs, and [interactive demo](https://huggingface.co/spaces/luxury-lakehouse/soccer-analytics-demo). Every artifact's README is auto-uploaded from the in-repo source of truth by `ingestion.hf_publish` (ADR-014). |
...
**SEC1 EU AI Act gap analysis cycle (2026-04-14)** — 16 Taipy pages, 40 synced tables (Lakebase reverse-ETL), 71 PG indexes (sub-second dashboard queries), 2,337 unit tests. Regulation (EU) 2024/1689 gap analysis is maintained in [`AI_GOVERNANCE.md`](AI_GOVERNANCE.md), covering 13 per-player evaluative ML systems with per-system risk classification, Article 11 / Annex IV technical-documentation mapping, Article 14 human-oversight mapping, and an Article 10 fairness analysis. Enforced via `src/tests/test_ai_governance_md.py` (section completeness + workflow-card inventory parity + 30-day review freshness). Every Hugging Face Hub model card carries an "EU AI Act — Intended Use and Non-Use" stanza. **Daily Job Hardening (2026-04-13)** — self-healing daily Databricks job: 27 ingestion tasks → `dbt_build` (gold materialization via wheel-bundled `ingestion.dbt_runner` python_wheel_task) → `refresh_synced_tables` (Lakebase propagation). SHA-256 artifact integrity verification on model loads (SEC-AUDIT ML-02). Hugging Face Hub: 17 models + 19 datasets published, GPU training on HF Jobs L40S, [Gradio demo Space](https://huggingface.co/spaces/luxury-lakehouse/soccer-analytics-demo) with luxury flagship theme (6 tabs). Football2vec v2: 128-dim transformer embeddings with adversarial team debiasing (Ganin GRL). Shape graph formation detection (Sotudeh 2026) + 5x5 position maps. xG v2 set encoder (ROC-AUC 0.915, MC dropout uncertainty). PSxG logistic model (Butcher et al. 2025) + Football2Vec 360 encoder (144-dim). ScoutGPT decoder + training pipeline (D32). 33 skip guards with mandatory `FilterResult` injection (centralized freshness gate removed in D52), entity count observability (D40). Mart classification taxonomy (PR-Cycle-C, ADR-019): every gold mart tagged for upcoming three-stage `dbt_build` (PR-α metadata; PR-β TF restructure). See [ARCHITECTURE.md](ARCHITECTURE.md) for the platform architecture and [ROADMAP.md](ROADMAP.md) for research directions.
