...
- **Scheduling:** 12 tables use `TRIGGERED` (CDF-based incremental sync); 29 use `SNAPSHOT` (on-demand refresh).
...
| Anzer & Bauer (2022) — Expected Passes | "Expected Passes." *Data Mining and Knowledge Discovery 36(1), 295-317* | xSuccess action-completion lineage (`xsuccess` in `fct_action_values`) — the sk `XSuccessModel` generalises expected-pass-completion to all on-ball action types; weights VAEP_adjusted, `wf-vaep` (silly-kicks TF-61) |
...
| Dash, Ide, Umemoto, Amino & Fujii (2025) | "Prediction-based evaluation of back-four defense with spatial control in soccer." *arXiv:2511.06191* | Rest-defense back-four line height / compactness / lateral spread (`rd_line_height`/`rd_compactness_x`/`rd_width`) in `fct_rest_defense`, tracking-marts drain (silly-kicks TF-14/TF-60) |
...
| Dixon & Coles (1997) | "Modelling Association Football Scores and Inefficiencies in the Football Betting Market." *Journal of the Royal Statistical Society: Series C 46(2)*. DOI: 10.1111/1467-9876.00065 | Match-outcome low-scoreline dependence correction (`p_win`/`p_draw`/`p_loss`/`xpoints`) in `fct_team_metrics`, `wf-match-outcome` (silly-kicks TF-53) |
...
| Forcher et al. (2023) | "The Success Factors of Rest Defense in Soccer." *Journal of Sports Science and Medicine (PMC10690503; Bundesliga 2020/21)* | Rest-defense KPI battery — numerical superiority behind the ball + attacker space-control (`rd_num_superiority`/`rd_attacker_space_control`) in `fct_rest_defense`, tracking-marts drain (silly-kicks TF-60; the DESCRIPTIVE KPIs only, not the paper's weak success model) |
...
| Glickman, M.E. (2012) | "Example of the Glicko-2 system." *Boston University* | Ground-duel Glicko-2 rating (`duel_rating`/`duel_rating_deviation`/`duel_volatility`) in `fct_player_match_metrics`, `wf-duels` (silly-kicks TF-55) |
...
| Sumpter / Twelve.football | Twelve match-report KPI glossary (PPDA, field tilt, pass tempo, line heights, build-up taxonomy) + the territorial-dominance / defensive-hull 'Earpiece' framing — practitioner vocabulary, David Sumpter / twelve.football (no single published paper) | Team-metrics KPIs (`ppda`, `field_tilt_pct`, `pass_tempo`, …) in `fct_team_metrics`, `wf-team-metrics` (silly-kicks TF-52); territorial-dominance (`territory_xt_conceded`/`territory_xt_prevented`) in `fct_player_match_metrics`, `wf-territory` (silly-kicks TF-54) |
