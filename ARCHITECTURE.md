...
- **Entity count observability**: `CostEstimateHook` writes `entity_count` (input entities from guard) alongside `row_count` (output rows) to `workflow_cost_live` in the observability schema. `fct_workflow_costs` uses `tasks` (lakeflow) as the driving table with LEFT JOIN on billing — timing data is available immediately, billing cost arrives with ~1 day lag. `effective_cost_usd = COALESCE(attributed_cost_usd, estimated_cost_usd)` ensures the UI always has a cost value. `cold_start_seconds` captures serverless environment setup overhead. Warm-tier join uses `workflow_id` + temporal window (D55: serverless exposes no `job_run_id`/`task_key` via Spark conf or env vars).
